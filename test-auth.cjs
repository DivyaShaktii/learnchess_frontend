...
let confirmed = false;
const supabase = {
  auth:{getSession:async()=>({data:{session:{access_token:'test-token'}}}),signUp:async()=>({data:{user:{id:'alice'},session:null},error:null})},
...
async function run() {
  ...
  const AuthForm=load('./src/components/AuthForm.tsx', {'react':react,'lucide-react':{},'../utils/supabaseClient':{supabase}}).default;
  states=[]; index=0; overrides=[false,'test@example.invalid','test-password','2000'];
  ...
  console.log('PASS: cached entitlement, profile outage, and unconfirmed signup regressions');
}
